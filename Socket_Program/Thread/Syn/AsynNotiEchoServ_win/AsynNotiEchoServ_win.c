    ...
    while (1) {
        ...
        for (size_t i = start_index; i < Numofsock; i++) {
            ...
            if (sigEventindex == WAIT_TIMEOUT || sigEventindex == WSA_WAIT_FAILED)
                ...
                sigEventindex = i;
                if (WSAEnumNetworkEvents(hClntSock[sigEventindex],
                                         hEvents[sigEventindex],
                                         &netEvent) == SOCKET_ERROR)
                ...
                if (netEvent.lNetworkEvents & FD_ACCEPT) {
                    ...
                    clnt_sock = accept(hClntSock[sigEventindex], (SOCKADDR*)&clntAddr, &szClntAddr);
                    ...
                    str_len = recv(hClntSock[sigEventindex], message, BUF_SIZE, 0);
                    send(hClntSock[sigEventindex], message, str_len, 0);
                }
                    ...
                    closesocket(hClntSock[sigEventindex]);
                    WSACloseEvent(hEvents[sigEventindex]);
                    printf("close client : %d\n", hClntSock[sigEventindex]);
                    CompressSockets(hClntSock, sigEventindex, Numofsock);
